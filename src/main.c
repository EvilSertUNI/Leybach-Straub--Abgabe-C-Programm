...
enum { sel_invalid =
        0, sel_developers, sel_group_logo, sel_group, sel_exit };
...
main(void)
{
    ...
    const Group_Logo logo =
        create_group_logo(group_caption, developer1_logo, developer2_logo);

    // Going into the programm (while-) loop (UML-diagram)
    while (1 == 1) {
        int             user_input;     // |:ws:| plain int is handled
                                        // most efficiently by CPU. No
                                        // need to use special integer
                                        // size
        int             input_count;    // |:ws:| prototype: int scanf()
                                        // return value: `number of input
        ...
        } while (input_count != 1);     // Repeats, until input is correct 
                                        // (In terms of type)
        ...
        // Produce error message if input number doesn't fit allowed
        // parameters
